...
GWIS._checkOpts = function _checkOpts( opts ) {
    ...
    $.each( opts.series, function(idx) {
        ...
        //must have either tsid or site
        if ( ser.tsid===undefined && ser.site===undefined)  { throw new Error(funcName+"series option 'site' OR 'tsid' is required to specify what site to retrieve data for"); }

        //check to make sure we dont have both tsid and site
        if ( ser.tsid && ser.site)  { throw new Error(funcName+"series option must only have a single major filter: 'site' OR 'tsid'"); }

        //..............
        //  tsid [string]
        //   USGS TSID
        //   this option is OPTIONAL and must be all digits
        if ( ser.tsid ) {
...
GWIS._createPlot = function ( opts ) {
    ...
    var plot = new Dygraph(
        ...
        { // options
            ...
            "interactionModel"             : Dygraph.defaultInteractionModel,      // adding range selector turns off click-drag-zoom by default - reset to turn back on
...
GWIS._plotError = function( opts, msg ) {
    ...
    if (GWIS._isLoadedJq && opts!==undefined && opts.div_id!==undefined) {
        $("#"+opts.div_id)
            ...
            .html( '<div class="gwis-center-me">Plot Unavilable</div>' );
        GWIS._center_mes();
    }
